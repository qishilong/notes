...
public:
    string longestPalindrome(string s) {
        ...
        // 模板：预处理hash
        preH = vector<long long>(n + 1, 0); // 前缀Hash
        preH[0] = 0;
        p131 = vector<long long>(n + 1, 0); // 131次幂
        p131[0] = 1;
        for (int i = 1; i <= n; i++) {
            preH[i] = (preH[i - 1] * 131 + (s[i] - 'a' + 1)) % p;
            p131[i] = p131[i - 1] * 131 % p;
        }

        sufH = vector<long long>(n + 2, 0); // 后缀Hash
        sufH[n + 1] = 0;
        for (int i = n; i; i--) {
            sufH[i] = (sufH[i + 1] * 131 + (s[i] - 'a' + 1)) % p;
        }
    ...
    vector<long long> preH, sufH, p131;
    const int p = 1e9 + 7;
    int n;
};